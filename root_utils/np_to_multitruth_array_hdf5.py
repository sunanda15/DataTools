 ...
 #   if git_status:
  ...
  #  f.attrs['timestamp'] = str(datetime.now())
    ...
    for input_file in config.input_files:
        ...
        for t in track_pid:
            total_tracks += t.shape[0]
    
    print(len(config.input_files), "files with", total_rows, "events with ", total_hits, "hits", "and with", total_tracks, "tracks")
    ...
    dset_veto2 = f.create_dataset("veto2",
                                  shape=(total_rows,),
                                  dtype=np.bool_)
    dset_event_track_index = f.create_dataset("event_track_index",
                                              shape=(total_rows,),
                                              dtype=np.int64)
    dset_track_vertex = f.create_dataset("track_vertex",
                                         shape=(total_tracks,),
                                         dtype=np.float32)
    dset_track_positions = f.create_dataset("track_positions",
                                           shape=(total_tracks,1,3),
                                           dtype=np.float32)
    dset_track_angles = f.create_dataset("track_angles",
                                        shape=(total_tracks,1,2),
                                        dtype=np.float32)
    dset_track_labels = f.create_dataset("track_labels",
                                         shape=(total_tracks,),
                                         dtype=np.int32)
    dset_track_parentid = f.create_dataset("track_parentid",
                                           shape=(total_tracks,),
                                           dtype=np.int32)
    dset_track_vertexid = f.create_dataset("track_vertexid",
                                           shape=(total_tracks,),
                                           dtype=np.int32)
    dset_track_idexitflag = f.create_dataset("track_idexitflag",
                                             shape=(total_tracks,),
                                             dtype=np.int32)
    dset_track_idexitpos = f.create_dataset("track_idexitpos",
                                            shape=(total_tracks,3),
                                            dtype=np.float32)
    dset_track_idexitekin = f.create_dataset("track_idexitekin",
                                            shape=(total_tracks,),
                                            dtype=np.float32)
    ...
    for input_file in config.input_files:
        ...
        track_parent_id = npz_file['track_parentid']
        track_vertex_id = npz_file['track_vertexid']
        track_idexitflag = npz_file['track_idexitflag']
        track_idexitpos = npz_file['track_idexitpos']
        track_idexitekin = npz_file['track_idexitekin']
