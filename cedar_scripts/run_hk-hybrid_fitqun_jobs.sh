...
for f in ${data_dir}/${name}/WCSim/{e-,mu-}/*/*/*/*_0.root; do
  p=${f##*/WCSim/}
  p=${p%%/*}
  for i in {0..300..300}; do # {0..2700..300}; do
    JOBTIME=`date` sbatch --time=1-00:00:00 --mem=8G --array=0-399\
     --job-name=fq${i}_${p} "/project/rpp-blairt2k/prouse/jobscripts/runfiTQun.sh"\
